...
import com.diandong.domain.dto.DictDishTypeExportDTO;
import com.diandong.domain.vo.DictDateVO;
import com.diandong.domain.vo.DictDishTypeVO;
 ...
 * @author ruoyi
 */
@Api(value = "/dict/dishes_type", tags = {"菜类设置"})
@RestController
@RequestMapping("/dict/dishes_type")
public class DictDishesTypeController extends BaseController {
    @Resource
    ...
    @PreAuthorize("@ss.hasPermi('dict:unit:list')")
    @GetMapping
    ...
    @PreAuthorize("@ss.hasPermi('dict:unit:export')")
    @PostMapping("/export")
    ...
    @PreAuthorize("@ss.hasPermi('dict:unit:query')")
    ...
    @PreAuthorize("@ss.hasPermi('dict:unit:add')")
    ...
    @PreAuthorize("@ss.hasPermi('dict:unit:edit')")
    ...
    @PreAuthorize("@ss.hasPermi('dict:unit:remove')")
...
//    @PostMapping("/export")
//    public void export(HttpServletResponse response, DictDishTypeVO vo) {
//        List<Long> ids = vo.getIds();
//
//
//        List<SysDictData> dictDataList;
//        List<DictDishTypeExportDTO> list = new ArrayList<>();
//        if (CollectionUtils.isNotEmpty(ids)) {
//            dictDataList = dictDataService.selectBatchByIds(ids);
//        } else {
//            SysDictData dictData = new SysDictData();
//            dictData.setDictLabel(vo.getDishTypeName());
//            dictDataList = dictDataService.selectDictDataList(dictData);
//        }
//        for (SysDictData sysDictData : dictDataList) {
//            DictDishTypeExportDTO dictDishTypeExportDTO = new DictDishTypeExportDTO();
//            dictDishTypeExportDTO.setDishTypeName(sysDictData.getDictLabel());
//            dictDishTypeExportDTO.setRemark(sysDictData.getRemark());
//
//            list.add(dictDishTypeExportDTO);
//        }
//        ExcelUtil<DictDishTypeExportDTO> util = new ExcelUtil<DictDishTypeExportDTO>(DictDishTypeExportDTO.class);
//        util.exportExcel(response, list, "菜品类别");
//    }
    ...
    private void resetSysDictData(SysDictData sysDictData) {
        ...
        if (StringUtils.isBlank(sysDictData.getDictType())) {
            sysDictData.setDictType(SysConstants.DISHES_TYPE);
        }
    }
}
