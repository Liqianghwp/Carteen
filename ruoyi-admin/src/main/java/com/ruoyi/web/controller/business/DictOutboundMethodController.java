 ...
 * @author ruoyi
 */
@Api(value = "/dict/outbound", tags = {"出库方式设置"})
@RestController
...
public class DictOutboundMethodController extends BaseController {
    ...
    @PreAuthorize("@ss.hasPermi('dict:outbound:list')")
    @GetMapping
    ...
    @PreAuthorize("@ss.hasPermi('dict:outbound:export')")
    @PostMapping("/export")
    ...
    @PreAuthorize("@ss.hasPermi('dict:outbound:query')")
    ...
    @PreAuthorize("@ss.hasPermi('dict:outbound:remove')")
    ...
    private void resetSysDictData(SysDictData sysDictData) {
        ...
        if (StringUtils.isBlank(sysDictData.getDictType())) {
            sysDictData.setDictType(SysConstants.OUTBOUND_METHOD);
        }
    }
}
