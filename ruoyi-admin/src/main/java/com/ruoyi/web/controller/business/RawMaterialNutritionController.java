...
public class RawMaterialNutritionController extends BaseController {
    ...
    @GetMapping
    public BaseResult getList(RawMaterialNutritionVO vo) {

        Page<RawMaterialNutritionPO> page = onSelectWhere(vo).page(new Page<>(vo.getPageNum(), vo.getPageSize()));
...
//
    ...
    @PutMapping
    public BaseResult update(@RequestBody @Validated(Update.class) RawMaterialNutritionVO vo) {
//        判断登录状态
        LoginUser loginUser = getLoginUser();
        if (Objects.isNull(loginUser)) {
            return BaseResult.error(Constants.ERROR_MESSAGE);
    ...
    private LambdaQueryChainWrapper<RawMaterialNutritionPO> onSelectWhere(RawMaterialNutritionVO vo) {

        LambdaQueryChainWrapper<RawMaterialNutritionPO> queryWrapper = rawMaterialNutritionMpService.lambdaQuery();
