 ...
 * @author ruoyi
 */
@Api(value = "/dict/warehouse", tags = {"仓库管理"})
@RestController
@RequestMapping("/dict/warehouse")
public class DictWarehouseManagementController extends BaseController {
    @Resource
    ...
    @PreAuthorize("@ss.hasPermi('dict:warehouse:list')")
    @GetMapping
    ...
    @PreAuthorize("@ss.hasPermi('dict:warehouse:query')")
    ...
    @PreAuthorize("@ss.hasPermi('dict:warehouse:add')")
    ...
    @PreAuthorize("@ss.hasPermi('dict:warehouse:edit')")
    ...
    @PreAuthorize("@ss.hasPermi('dict:warehouse:remove')")
    ...
    private void resetSysDictData(SysDictData sysDictData) {
        ...
        if (StringUtils.isBlank(sysDictData.getDictType())) {
            sysDictData.setDictType(SysConstants.WAREHOUSE_MANAGEMENT);
        }
    }
}
