 ...
 * @author ruoyi
 */
@Api(value = "/dict/sample", tags = {"留样存储地点"})
@RestController
@RequestMapping("/dict/sample")
public class DictSampleStorageController extends BaseController {
    @Resource
    ...
    @PreAuthorize("@ss.hasPermi('dict:sample:list')")
    @GetMapping
    ...
    @PreAuthorize("@ss.hasPermi('dict:sample:query')")
    ...
    @PreAuthorize("@ss.hasPermi('dict:sample:add')")
    ...
    @PreAuthorize("@ss.hasPermi('dict:sample:edit')")
    ...
    @PreAuthorize("@ss.hasPermi('dict:sample:remove')")
    ...
    private void resetSysDictData(SysDictData sysDictData) {
        ...
        if (StringUtils.isBlank(sysDictData.getDictType())) {
            sysDictData.setDictType(SysConstants.SAMPLE_STORAGE);
        }
    }
}
