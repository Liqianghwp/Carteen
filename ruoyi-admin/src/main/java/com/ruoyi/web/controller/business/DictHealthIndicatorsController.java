...
public class DictHealthIndicatorsController extends BaseController {
    ...
    @PreAuthorize("@ss.hasPermi('dict:health:export')")
    @PostMapping("/export")
    ...
    @PreAuthorize("@ss.hasPermi('dict:health:query')")
    ...
    @PreAuthorize("@ss.hasPermi('dict:health:remove')")
    ...
    private void resetSysDictData(SysDictData sysDictData) {
        ...
        if (StringUtils.isBlank(sysDictData.getDictType())) {
            sysDictData.setDictType(SysConstants.HEALTH_INDICATORS);
        }
    }
}
